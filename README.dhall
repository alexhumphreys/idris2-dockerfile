...
```
docker pull snazzybucket/idris2:${env:DOCKER_TAG as Text}
```
...
If there's been no commits to master, the images are still rebuilt. I'm not sure if the docker image SHAs will remain the same, but if you pull say tag `:${env:DOCKER_TAG as Text}` twice and get different SHAs, that's probably what happened.
