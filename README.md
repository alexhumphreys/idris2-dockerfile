...
There is also docker tags in the [`git describe --tags` (link to docs)](https://git-scm.com/docs/git-describe) format, eg:

```
docker pull snazzybucket/idris2:v0.5.1-341-g7a5f63ea
```
...
If there's been no commits to master, the images are still rebuilt. I'm not sure if the docker image SHAs will remain the same, but if you pull say tag `:v0.5.1-341-g7a5f63ea` twice and get different SHAs, that's probably what happened.
