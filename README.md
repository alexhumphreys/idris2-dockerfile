...
```
docker pull snazzybucket/idris2:v0.6.0
```
...
If there's been no commits to master, the images are still rebuilt. I'm not sure if the docker image SHAs will remain the same, but if you pull say tag `:v0.6.0` twice and get different SHAs, that's probably what happened.
